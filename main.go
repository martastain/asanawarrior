...
import (
    ...
    "github.com/martastain/asanawarrior/asana"
    "github.com/martastain/asanawarrior/taskwarrior"
    "github.com/martastain/asanawarrior/x"
    "github.com/pkg/errors"
)

var duration = flag.Int("dur", 1, "How often to run sync, specified in minutes.")
var run_once = flag.Int("once", 0, "Run one and exit (for cron)")
var dbpath = flag.String("db", os.Getenv("HOME")+"/.task/asanawarrior.db", "File path for db which stores certain sync information.")
var notifyInterval = flag.Int("interval", 10, "Minimum duration in seconds between successive notifications. Set to zero for no notifications.")
var maxDeletes = flag.Int("deletes", 5, "If Asanawarrior sees more than these number of deletes, it's going to crash to protect your Asana from mass deletion.")
...
func main() {
    flag.Parse()
    fmt.Println("Asanawarrior v1.1 - Bringing the power of Taskwarrior to Asana")
